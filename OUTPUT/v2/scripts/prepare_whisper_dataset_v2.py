#!/usr/bin/env python3
"""
Whisper dataset preparation pipeline (v2).

This variant mirrors scripts/prepare_whisper_dataset.py but writes every
generated artifact (JSONL splits, summaries, optional normalised audio,
combined dataset) into the ./v2 subtree so we can keep versioned exports
separate from the legacy outputs.

Usage
-----
    python v2/scripts/prepare_whisper_dataset_v2.py \
        --source-root /home/makhosi/Hdrive/SPEECH_AUDIO_DATA/OUTPUT \
        --lowercase \
        --train-ratio 0.9
"""
...
class LanguageProcessor:
    def __init__(
        ...
        self.source_lang_dir = lang_dir
        self.output_lang_dir = output_root / lang_dir.name
        self.lang_code = lang_code
        self.args = args
        self.source_audio_dir = self.source_lang_dir / "audio"
        self.target_audio_dir = self.output_lang_dir / args.audio_out if args.normalize_audio else None
        self.source_trans_dir = self.source_lang_dir / "transcriptions"
        self.output_trans_dir = self.output_lang_dir / "transcriptions"
        self.output_train = self.output_trans_dir / "train.jsonl"
        self.output_val = self.output_trans_dir / "val.jsonl"
        self.audio_index = AudioIndex(source_root, self.source_lang_dir)
    ...
    def run(self) -> LanguageSummary:
        if not self.source_trans_dir.exists():
            return self._empty_summary()
        self.output_trans_dir.mkdir(parents=True, exist_ok=True)
        if self.target_audio_dir:
            self.target_audio_dir.mkdir(parents=True, exist_ok=True)
        ...
        return LanguageSummary(
            language_dir=self.source_lang_dir.name,
            ...
            output_train=self.output_train.relative_to(self.output_root).as_posix(),
            output_val=self.output_val.relative_to(self.output_root).as_posix(),
        )

    def _split_and_write(self) -> Tuple[List[Example], List[Example]]:
        entries = sorted(self.examples.values(), key=lambda item: item.audio)
        rng = random.Random(f"{self.source_lang_dir.name}-{self.args.seed}")
        rng.shuffle(entries)
    ...
    def _load_transcripts(self) -> Iterator[Tuple[str, str]]:
        for file_path in sorted(self.source_trans_dir.rglob("*")):
            if not file_path.is_file():
                continue
    ...
    def _parse_two_column(self, file_path: Path, delimiter: str) -> Iterator[Tuple[str, str]]:
        with file_path.open("r", encoding="utf-8", errors="ignore") as f:
            ...
            for row in reader:
                ...
                if header_detected:
                    audio_ref = self._extract_by_header(
                        row,
                        header,
                        ("audio", "audio_file", "filepath", "file", "filename", "utt", "utterance", "utterance_id"),
                    )
    ...
    def _prepare_audio_target(self, rel_path: Path) -> Path:
        ...
        if not self.args.normalize_audio or not self.target_audio_dir:
            return self._relative_to_output_root(source_path)
        relative = source_path.relative_to(self.source_lang_dir) if source_path.is_relative_to(self.source_lang_dir) else Path(source_path.name)
        target = (self.target_audio_dir / relative).with_suffix(".wav")
        ...
        return self._relative_to_output_root(target)

    def _relative_to_output_root(self, absolute: Path) -> Path:
        absolute = absolute.resolve()
        try:
            return absolute.relative_to(self.output_root)
        except ValueError:
            rel = os.path.relpath(absolute, self.output_root)
            return Path(rel)
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Prepare Whisper JSONL splits under ./v2 for all language folders.")
    parser.add_argument("--source-root", type=Path, required=True, help="Dataset root (contains <LANG>-ZA directories).")
    ...
    parser.add_argument("--normalize-audio", action="store_true", help="Convert audio to 16 kHz mono WAV (outputs into ./v2).")
    parser.add_argument("--audio-out", type=str, default="audio_16k", help="Subdirectory name for normalised audio (relative to ./v2/<LANG>-ZA>).")
    parser.add_argument("--output-root", type=Path, help="Override output directory (default: <source-root>/v2).")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if not source_root.exists():
        print(f"[ERROR] Source root directory does not exist: {source_root}", file=sys.stderr)
        return 1
    output_root = args.output_root.resolve() if args.output_root else (source_root / "v2")
    ...
    if not language_dirs:
        print(f"[WARN] No matching <LANG>-ZA directories found under {source_root}")
        return 0

    summaries: List[LanguageSummary] = []
    combined_dataset_path = output_root / "dataset.jsonl"
    combined_entries: List[Example] = []
    for lang_dir in language_dirs:
        lang_code = LANG_CODE_MAP.get(lang_dir.name, lang_dir.name.split("-")[0])
        processor = LanguageProcessor(source_root, output_root, lang_dir, lang_code, args)
        summary = processor.run()
        summaries.append(summary)
        combined_entries.extend(processor.examples.values())
        print(
            f"[INFO] {lang_dir.name}: {summary.total_examples} examples "
    ...
    if not args.dry_run:
        ...
        print(f"[INFO] Summary written to {summary_path.relative_to(output_root).as_posix()}")
    else:
        print(f"[INFO] Dry-run: summary would be written to {summary_path.relative_to(output_root).as_posix()}")
    if combined_entries:
        if args.dry_run:
            print(f"[INFO] Dry-run: combined multilingual file would be at {combined_dataset_path.relative_to(output_root).as_posix()}")
        else:
            print(f"[INFO] Combined multilingual file: {combined_dataset_path.relative_to(output_root).as_posix()}")
    return 0
